#!/bin/bash
# Master script to align GATA CUT&Tag data
...
help() {
   echo "Builds masked mm10 genome and aligns CUT&Tag data using Bowtie2. Calls peaks and creates browser tracks."
   echo
   echo "Syntax: ./master1_CUTnTag_processing.sh [-f|d|p|h]"
   echo "options:"
   echo "f     Provide directory containing FASTQ files (GATA CUT&Tag). [mandatory]"
...
if [[ $path == '' ]]
then
	echo -e "Please provide the path to /Gata_paper/CUT&Tag_analysis/ with -p"
  exit 1
fi

if [[ $fastq_dir == '' ]]
then
	echo -e "Please provide the path to a directory containing CUTnTag screen FASTQ files with -f"
  exit 1
fi
...
echo -e "Build Masked mm10 genome for Bowtie2"
...
echo -e "Call peaks from CUT&Tag"
