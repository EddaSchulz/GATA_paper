#!/bin/bash
#Aligns Wu 2016 embryo ATAC-seq data to mm10
...
do
	...
	bowtie2 --local --very-sensitive -X 2000 -x $ebwt -1 $f\_1_val_1.fq -2 $f\_2_val_2.fq -S $f\.sam >> $f\.mappingStats.txt 2>&1
	rm $f\_1_val_1.fq
	rm $f\_2_val_2.fq

	echo -e "Removing mitochondrial reads from $f"
