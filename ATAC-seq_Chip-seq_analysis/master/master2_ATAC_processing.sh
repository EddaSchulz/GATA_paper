#!/bin/bash
# Master script to align Wu 2016 ATAC-seq
...
help() {
   echo "Builds mm10 genome and aligns Wu 2016 8-cell ATAC-seq data using Bowtie2. Calls peaks and creates browser tracks."
   echo
   echo "Syntax: ./master2_ATAC_processing.sh [-f|d|p|h]"
   echo "options:"
   echo "f     Provide directory containing FASTQ files (Wu 2016 8-cell ATAC-seq data). [mandatory]"
...
if [[ $fastq_dir == '' ]]
then
	echo -e "Please provide the path to a directory containing Wu 2016 8-cell ATAC-seq FASTQ files with -f"
  exit 1
fi
...
echo -e "Align ATACseq data"
${path}scripts/ATAC_align.sh $path $fastq_dir $work_dir $ebwt

bam_dir=${work_dir}final_bam'/'

# Calls peaks for replicates and merges them
echo -e "Call peaks from ATAC"
